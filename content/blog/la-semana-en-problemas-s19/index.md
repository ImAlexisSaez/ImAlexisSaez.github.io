...
Esta semana se proponen algunos enunciados relacionados con series de potencias
y representación de funciones.
...
Calcule el polinomio de Taylor de la función $f(x) = \ln{x}$ centrado en
$x_0 = 1$.
...
- (a) Sustituya el exponente $k$ por el mayor entero compatible con la condición
  de ser convergente la serie $\sum_{n=1}^{\infty}{a_n}$.
...
- (a) Pruebe que $$1 + x + x^2 + \ldots + x^{n-1} = \frac{1 - x^n}{1 - x}$$ para
  cualquier $n$ natural y deduzca que
  $$\sum_{n=0}^{\infty}{x^n} = \frac{1}{1 - x}$$ y determine el radio de
  convergencia de la anterior identidad.
- (b) Desarrolle en serie de potencias la función $$f(x) = \frac{3}{2 + 5x^2}$$
  y determine su radio de convergencia.
- (c) Calcule $f^{(n)}(0)$.
- (d) Plantee un problema contextualizado con diferentes apartados a partir de
  la identidad del punto (a). Resuelva el problema indicando todos los
  conocimientos previos necesarios.

### Ejercicio 5

Escriba el desarrollo de Taylor de la función $f(x) = (\log{x})^2$ con diversas
fórmulas del término complementario.

### Ejercicio 6

Haciendo uso de la fórmula de Taylor para la función $$f(x) = (1+x)^{1/3}$$
calcule aproximadamente $1.3^{1/3}$, situando el término complementario en el
lugar de las derivadas terceras. Estime el error cometido.
...
- La ecuación $f(x) = 0$ tiene exactamente una solución negativa, que además es
  única.
...
- (a) Haga la representación gráfica de la función sabiendo que es una función
  racional.
